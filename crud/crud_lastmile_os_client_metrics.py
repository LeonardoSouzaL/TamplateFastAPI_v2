...
class CRUDLastMileOsClientMetrics:
    """Consultas agregadas somente leitura de OS LastMile por cliente."""

    def _type_condition(self, filters: LastMileOsClientFilters):
    ...
    def _vision_conditions(self, filters: LastMileOsClientFilters):
    ...
    def aggregate_by_client_stmt(self, filters: LastMileOsClientFilters):
    ...
    def aggregate_travel_status_stmt(self, filters: LastMileOsClientFilters):
        latest_travel = latest_travel_subquery()
        ...
        return (
            ...
            .where(and_(*self._vision_conditions(filters)))
            .group_by(ServiceOrder.client_id, status_key)
        )

    async def aggregate_by_client(
        self,
        db: AsyncSession,
        *,
        ...
        result = await db.execute(self.aggregate_by_client_stmt(filters))
        return [dict(row) for row in result.mappings().all()]

    async def aggregate_travel_status_by_client(
        self,
        db: AsyncSession,
        *,
        filters: LastMileOsClientFilters,
...
crud_lastmile_os_client_metrics = CRUDLastMileOsClientMetrics()
